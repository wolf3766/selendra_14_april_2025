...
def generate_report(report_id):
    try:
        ...
        for store_id, timezone in stores:
            ...
            end_time = max_timestamp.astimezone(tz) 
            start_times = {
                'last_hour': (end_time - timedelta(hours=1)).astimezone(tz),
                'last_day': (end_time - timedelta(days=1)).astimezone(tz),
                'last_week': (end_time - timedelta(weeks=1)).astimezone(tz),
            }
        ...
        print(e)
        JobStore.objects.filter(job_id=report_id).update(status="failed")

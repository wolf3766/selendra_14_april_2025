...
### Overlapping Time Range Logic
Let’s say we are calculating uptime/downtime within a specific time range, for example, **10:00 AM to 12:00 PM**.
An interval is considered **overlapping** with this range if:
range_start <= interval_end AND range_end >= interval_start
 
So, for an interval like **9:00 AM to 1:00 PM**, this condition holds true because:

1. **10:00 AM** (range start) is less than or equal to **1:00 PM** (interval end), and  
2. **12:00 PM** (range end) is greater than or equal to **9:00 AM** (interval start)  
3. Hence, this interval **overlaps** with the 10:00 AM – 12:00 PM range.


Using this logic, any interval that overlaps even partially with the given time window will be included.
...
5. ## what can be improved?
   ...
   3. can use queue model with celery such that each worker only listens for tasks in a particular queue and can be used for all type of async processing based on priority.
